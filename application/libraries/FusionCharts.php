...
function renderChartHTML($chartSWF, $strURL, $strXML, $chartId, $chartWidth, $chartHeight) {
	...
	<OBJECT classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase=http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,0,0"  width="$chartWidth" height="$chartHeight" id="$chartId">
		...
		<embed src="$chartSWF" FlashVars="$strFlashVars" quality="high" wmode="opaque" width="$chartWidth" height="$chartHeight" name="$chartId" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
	</object>
...
#############============ For Future Use (end)============================
    ...
    function FusionCharts($param) {

        // modifikasi
        $chartType = $param['type'];
        $width = $param['width'];
        $height = $param['height'];
        $chartID = $param['ID'];
        ...
        $this->SWFPath="";
        if(isset($this->chartSWF[$this->chartType][0])) {
...
##------------ PRIVATE FUNCTIONS ----------------------------------------------------------------
    ...
    function getSeriesType() {
        $sValue=1;
        if(isset($this->chartSWF[$this->chartType])) {
    ...
    */
	...
	| Description	: Method untuk menentukan chart tipe apa yang akan digunakan
	| -------------------------------------------------------------------------------------------
    */
    function UseChart($name) {
        return BASEURL . "include/charts/".$name.".swf";
    }
