...
function UseMap($name){
	return BASEURL . "include/maps/".$name.".swf";
}
...
// renderMaps renders the JavaScript + HTML code required to embed a Maps.
...
function renderMap($MapsSWF, $strURL, $strXML, $MapsId, $MapsWidth, $MapsHeight, $debugMode, $registerWithJS) {
...
//renderMapsHTML function renders the HTML code for the JavaScript. This
...
// $MapsHeight - Intended height for the Maps (in pixels)
// $debugMode - Whether to start the Maps in debug mode
function renderMapsHTML($MapsSWF, $strURL, $strXML, $MapsId, $MapsWidth, $MapsHeight, $debugMode) {
...
$HTML_Maps = <<<HTMLMaps
	...
	<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0" width="$MapsWidth" height="$MapsHeight" id="$MapsId">
		...
		<embed src="$MapsSWF" FlashVars="$strFlashVars" quality="high" wmode="opaque" width="$MapsWidth" height="$MapsHeight" name="$MapsId" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
	</object>
