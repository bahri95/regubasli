...
tinymce.create('static tinymce.util.JSON',{
    serialize:function(o){
        ...
        if(t=='object'){
            ...
            v='{';
            for(i in o)v+=typeof o[i]!='function'?(v.length>1?',"':'"')+i+'":'+s(o[i]):'';return v+'}';
        }
        return''+o;
    },
    parse:function(s){
        ...
        if(x){
            ...
            function ready(){
                if(!o.async||x.readyState==4||c++>10000){
                    if(o.success&&c<10000&&x.status==200)o.success.call(o.success_scope,''+x.responseText,x,o);
                    else if(o.error)o.error.call(o.error_scope,c>10000?'TIMED_OUT':'GENERAL',x,o);
                    x=null;
...
setAttrib:function(e,n,v){
    ...
    setAttribs:function(e,o){
        ...
        break;
        case'compact':case'noshade':case'nowrap':
        if(v===65535)return n;
        return dv;
        case'shape':
        v=v.toLowerCase();
...
(function(){
    ...
    tinymce.create('static tinymce.dom.Event',{
        ...
        _add:function(o,n,f){
            ...
            else if(o.addEventListener)o.addEventListener(n,f,false);else o['on'+n]=f;
        },
        _remove:function(o,n,f){
            if(o){
                try{
                    if(o.detachEvent)o.detachEvent('on'+n,f);
                    else if(o.removeEventListener)o.removeEventListener(n,f,false);else o['on'+n]=null;
                }catch(ex){}
            }
        },
        _pageInit:function(){
...
(function(){
    ...
    tinymce.create('tinymce.dom.ScriptLoader',{
        ...
        loadQueue:function(cb,s){
            ...
            }else if(cb)t.queueCallbacks.push({
                func:cb,
                scope:s||t
            });
        },
...
tinymce.create('tinymce.ui.Toolbar:tinymce.ui.Container',{
    renderHTML:function(){
        ...
        for(i=0;i<cl.length;i++){
            ...
            if(pr&&co.ListBox){
                if(pr.Button||pr.SplitButton)h+=dom.createHTML('td',{
                    'class':'mceToolbarEnd'
                },dom.createHTML('span',null,'<!-- IE -->'));
            }
            if(dom.stdMode)h+='<td style="position: relative">'+co.renderHTML()+'</td>';else h+='<td>'+co.renderHTML()+'</td>';
            if(nx&&co.ListBox){
                if(nx.Button||nx.SplitButton)h+=dom.createHTML('td',{
    ...
    tinymce.create('tinymce.Editor',{
        ...
        init:function(){
            ...
            if(tinymce.relaxedDomain){
                if(isIE||(tinymce.isOpera&&parseFloat(opera.version())>=9.5))u='javascript:(function(){document.open();document.domain="'+document.domain+'";var ed = window.parent.tinyMCE.get("'+t.id+'");document.write(ed.iframeHTML);document.close();ed.setupIframe();})()';
                else if(tinymce.isOpera)u='javascript:(function(){document.open();document.domain="'+document.domain+'";document.close();ed.setupIframe();})()';
            }
                ...
                switch(k){
                    ...
                    case'paste':
                        Event.add(t.getBody(),k,function(e){
                            ...
                            else if(tinymce.isIE)tx=t.getWin().clipboardData.getData('Text');
                            eventHandler(e,{
                                text:tx,
                                html:h
                            });
            ...
            if(s.custom_shortcuts){
                ...
                function find(e){
                    ...
                    each(t.shortcuts,function(o){
                        if(o.ctrl!=e.ctrlKey&&(!tinymce.isMac||o.ctrl==e.metaKey))return;
                ...
                if(o.get){
                    ...
                    each(t.dom.select('table,span',o.node).reverse(),function(n){
                        if(n.nodeName=='TABLE'){
                            ...
                            else if(n.style.textDecoration=='line-through')na='strike';else na='';
                            if(na){
    ...
    tinymce.create('tinymce.EditorCommands',{
        ...
        execCommand:function(cmd,ui,val){
            ...
            switch(cmd){
                case'Cut':case'Copy':case'Paste':
                    ...
                    return true;
                case'mceResetDesignMode':case'mceBeginUndoLevel':
                    return true;
                case'unlink':
                    t.UnLink();
                    return true;
                case'JustifyLeft':case'JustifyCenter':case'JustifyRight':case'JustifyFull':
                    ...
                    return true;
                case'mceEndUndoLevel':case'mceAddUndoLevel':
                    ed.undoManager.add();
                    return true;
                default:
                    f=this[cmd];
        ...
        FontName:function(u,v){
            ...
            if(!v){
                ...
                if(ed.settings.convert_fonts_to_spans)t._applyInlineStyle('span',{
                    style:{
                        fontFamily:v
                    }
        ...
        FontSize:function(u,v){
            ...
            if(v>=1&&v<=7)ed.getDoc().execCommand('FontSize',false,v);else this._applyInlineStyle('span',{
                style:{
                    fontSize:v
                }
            });
        ...
        RemoveFormat:function(){
            ...
            if(isWebKit)s.setContent(s.getContent({
                format:'raw'
        ...
        mceSetStyleInfo:function(u,v){
            ...
            re=ed.settings.merge_styles_invalid_parents;
            if(tinymce.is(re,'string'))re=new RegExp(re,'i');
            if((e=t.getSelectedElement())&&!ed.settings.force_span_wrappers)set(e,1);
            else{
...
(function(){
    ...
    tinymce.create('tinymce.ControlManager',{
        ...
        createListBox:function(id,s,cc){
            ...
            id=t.prefix+id;
            if(ed.settings.use_native_selects)c=new tinymce.ui.NativeListBox(id,s);
            else{
